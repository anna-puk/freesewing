...
function boundsIntersect(aStart, aEnd, bStart, bEnd, cStart, cEnd, dStart, dEnd, angleBetween, tol) {
  ...
  const xACa =
    ...
    aStart.shiftFractionTowards(
      aEnd,
      closestFraction(
        projectPointOnBeam(aStart, aEnd, angleBetween < 0 ? cStart : cEnd, tol)
      )
    )
  const xBDb =
    xBD ||
    bStart.shiftFractionTowards(
      bEnd,
      closestFraction(
        projectPointOnBeam(bStart, bEnd, angleBetween < 0 ? dEnd : dStart, tol)
      )
    )    
  ...
  const xACc =
    ...
    cStart.shiftFractionTowards(
      cEnd,
      closestFraction(
        projectPointOnBeam(cStart, cEnd, angleBetween < 0 ? aStart : aEnd, tol)
      )
    )
  const xBDd =
    xBD ||
    dStart.shiftFractionTowards(
      dEnd,
      closestFraction(
        projectPointOnBeam(dStart, dEnd, angleBetween < 0 ? bEnd : bStart, tol)
      )
    )    
...
export function lineIntersectsCurveAlt(start, end, from, cp1, cp2, to, tol, depth = 0, segmentName = 'base') {
  ...
  if (depth < maxIterations) {
    ...
    console.log('lengthsToRemove', lengthsToRemove, 'lengthAB:',bound_a.length(),'remaining:',bound_a.length() - (lengthsToRemove[0] + lengthsToRemove[1]),'lengthCD:',bound_c.length(),'remaining:',bound_c.length() - (lengthsToRemove[2] + lengthsToRemove[3]))

    if (!lengthsToRemove) {
      // no overlap between the bounding boxes ==> no intersection
      console.log('no intersection found (and parallel bounds) after', depth, 'for segment', segmentName, 'iterations')
      return false
    }
    ...
    if (newLength < tol) {
      ...
      const curvePointsA = [new Point(halves.left.points[0].x, halves.left.points[0].y), new Point(halves.left.points[1].x, halves.left.points[1].y), new Point(halves.left.points[2].x, halves.left.points[2].y), new Point(halves.left.points[3].x, halves.left.points[3].y)]
      const curvePointsB = [new Point(halves.right.points[0].x, halves.right.points[0].y), new Point(halves.right.points[1].x, halves.right.points[1].y), new Point(halves.right.points[2].x, halves.right.points[2].y), new Point(halves.right.points[3].x, halves.right.points[3].y)]
...
export function curvesIntersectAlt(fromE, cp1E, cp2E, toE, fromF, cp1F, cp2F, toF, tol, depth = 0, segmentName = 'base') {
  const maxIterations = 10
  let intersections,
    tempOp,
    temp_points,
  ...
  if (depth < maxIterations) {
    ...
    if (newCurveE.length() < 10 * tol) {
      ...
      if (tempPointE.dist(tempPointF) < tol) {
        ...
        console.log('found intersection for segment', segmentName,
          'in ', depth, 'iterations by reducing',(depth % 2 > 0) ? 'second' : 'first','curve length to <10*tol')
        return intersections
      }
    }
    ...
    if (newLength < tol) {
      ...
      const closestPointOnCurveE = bzE.project({x: potentialIntersection.x, y: potentialIntersection.y})
      
      console.log('hoping to find intersection for segment', segmentName, ' in ', depth, 'iterations using potential intersection',potentialIntersection,'and closest point',closestPointOnCurveE)
      
      if (potentialIntersection.dist(closestPointOnCurveE) < tol) {
        intersections = potentialIntersection // got it!
        console.log(
          'found intersection for segment', segmentName, ' in ', depth, 'iterations by reducing',(depth % 2 > 0) ? 'first' : 'second','curve length to <tol'
        )
        return intersections
      } else {
        console.log('ruled out intersections for segment', segmentName, ' in ', depth, 'iterations by reducing',(depth % 2 > 0) ? 'first' : 'second','curve length to <tol')
      ...
      // split, then repeat the whole thing (recursive function)
      console.log('splitting',(depth % 2 > 0) ? 'first' : 'second',' curve for segment', segmentName, ' in half after ', depth, ' iterations; new length: ', newLength, '; old length: ',oldLength)
      ...
      // add any intersections from the first part
      intersections.push(
        curvesIntersectAlt(...curvePointsA, newFromE, newCp1E, newCp2E, newToE, tol, depth + 1, segmentName + 'left')
      )

      // add any intersections from the second part
      intersections.push(
        curvesIntersectAlt(...curvePointsB, newFromE, newCp1E, newCp2E, newToE, tol, depth + 1, segmentName + 'right')
      )
